...
void DecodeSystemInfo1(const Trio_trace_frame * pTraceFrame)
{
    if (pTraceFrame != NULL)
    {
        uint8_t * pMsgCode = pTraceFrame->data;
...
void ProcessRRMessage(int64_t MessageTypeCode, const Trio_trace_frame * pTraceFrame)
{
    ...
    const uint8_t * gsmFrame = pTraceFrame->data;
    int iMsgCodeLen = pTraceFrame->dataLen;
        ...
        switch (iMessageType)//MessageType
        {
            ...
            case 0x28://切换失败
                if (s_bStartHandover)
                {
                    ...
                    s_Handover.HandOverEndTimestamp= getRealtimeOfCS();
            ...
            case 0x2b://切换命令
                ...
                s_Handover.HandoverStartTimestamp = getRealtimeOfCS();
...
void PreprocessL3Message(Trio_trace_frame * pTraceFrame)
{
    ...
    uint8_t * pMsgCode = pTraceFrame->data;
    int iMsgLen = pTraceFrame->dataLen;
    if (iMsgLen >= 1)
    {
