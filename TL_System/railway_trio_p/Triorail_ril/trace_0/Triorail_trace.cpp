...
const char * s_device_path_trace = "/dev/ttymxc3";  //ttymxc2
#else
...
static void getTraceFrame()
{
    ...
    for (;;) {
		...
		RLOGI("getTraceFrame trace_stream_get_next=%d", ret);
...
*/
    ...
    for (;;) {
        ...
        RLOGI("readTraceStreamLoop select=%d", n);  
        if (n < 0) {
...
*/
    ...
    for (;;) {
        ...
        while (traFrame != &s_traceFrameList) {
            ...
            RLOGI("processTraceFrameLoop before process.");
            //parser trace frame.
            processOTRByTypes(traFrame);
            RLOGI("processTraceFrameLoop after process.");
...
*/
    ...
    switch (cmd) {
        ...
        case TRACE_ON:
            {
                ...
                RLOGE("begin to process trace stream.");
        ...
        case TRACE_SHORT_STATISTIC_IND:
            RLOGE("TRACE_SHORT_STATISTIC_IND.");
            if (s_trace_open){
