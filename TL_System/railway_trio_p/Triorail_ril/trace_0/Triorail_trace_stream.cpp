                                            ...
                                            unsigned char *p_end, int & dummy_len)
{
    ...
    dummy_len = 0;
    RLOGI ("getEndOfFrame begin.");
    while (p_begin < p_end){
        if (p_end < p_begin + HEADER_SIZE) {
            return NULL;
        }
        inputLen = p_end - p_frameBegin;
		RLOGI ("getEndOfFrame inputLen=%d.", inputLen);
		...
		RLOGI ("getEndOfFrame appMsgLen=%d", appMsgLen);
        ...
        if (p_begin[appMsgLen+1] == 0x03) {
			RLOGI ("getEndOfFrame  get ETX");
            xorValue = frameCheckSum(p_frameBegin, appMsgLen + 3, 1);
            RLOGI ("getEndOfFrame xorValue=0x%02x", xorValue);
...
static void *getNextFrame (TraceStream *p_rs, size_t *p_outRecordLen)
{
    ...
    RLOGI ("getNextFrame after getEndOfFrame dummy_len=%d", dummy_len);
    ...
    if (record_end != NULL) {
        ...
        *p_outRecordLen = record_end - record_start;
		RLOGI ("getNextFrame p_outRecordLen=%zu",  *p_outRecordLen);
        return record_start;
    }
                                    ...
                                    size_t *p_outRecordLen)
{
	...
	*p_outRecordLen =  50;
    ...
    if (p_rs->unconsumed != p_rs->buffer) {
		...
		RLOGI ("frame_stream_get_next  toMove: %d:\n", toMove);
    ...
    RLOGI ("frame_stream_get_next  countRead: %zd:\n", countRead);
#ifdef debug_trace_stream        
