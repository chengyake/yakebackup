...
void print_local_frame(struct cmd_header *p) {
    ...
    syslog(LOG_INFO, "host:%d target:%d cmd:%d len:%d", p->host, p->target, p->cmd, p->len);
    for(i=0; i<p->len; i++) {
...
int write_msg(int fd, char *buf, int len) {
    ...
    if(ret < 0) {
        syslog(LOG_ERR, "write msg error: %s", strerror(ret));
        return ret;
    }
    return ret;
}
...
static int recv_and_process(int *fd) {
    ...
    if(*fd == 0) {
        syslog(LOG_ERR, "read core server error: fd is 0");
        return 0;
    }

    sum = recv(*fd, &buffer[len], SOCKET_BUFFER_SIZE, 0);
    if(sum <= 0) {
        syslog(LOG_ERR, "read core server error1 %d", sum);
        close(*fd);
        *fd=0; 
...
int clients_connection_changed() {
    if (FD_ISSET(server_fd, &fd_sets)) {
        ...
        if (get_clients_count() < IDX_CLIENTS_MAX) {
            ...
            syslog(LOG_INFO, "add client_fd: count = %d", get_clients_count());
        } else {
            syslog(LOG_ERR, "too many connections attached, ignore this one: fd num = %d", get_clients_count());
            close(new_fd);
        }
    }
}
...
static void show_clients() {
    ...
    int i;
    syslog(LOG_INFO, "fd amount = %d", get_clients_count());
    for (i = 0; i < IDX_CLIENTS_MAX; i++) {
        syslog(LOG_INFO, "fd[%d] = %d", i, fd_clients[i]);
    }
}
...
int setup_event_loop() {
    ...
    while (core_event_loop) {
        ...
        if (ret < 0) {
            syslog(LOG_ERR, "select error %d", ret);
            break;
        } else if (ret == 0) {
            syslog(LOG_ERR, "select timeout err %d", ret);
            continue;
        }
