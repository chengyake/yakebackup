...
int core_status_rsp(struct cmd_header *chp) {
    ...
    if(fdp == NULL) {
        logerr("%s error: fd = NULL", __func__);
        return -1;
    }
...
int to_core_event(int fd, struct cmd_header *chp) {
    ...
    switch(chp->cmd) {
        ...
        case CORE_MSG_REGISTER_CMD:
            fdp = get_fd_by_module(chp->host);
            if(fdp == NULL) {
                logerr("to core event error: fd = NULL");
                return -1;
            }
            *fdp = fd;
            //rsp ack to module
        ...
        default:
            logerr("to core event error: no cmd detected");
            return -1;
            break;
    }
    return 0;
...
int to_others_event(struct cmd_header *chp) {
    ...
    if(fdp == NULL) {
        logerr("to module %d event error: fd = NULL", chp->target);
        return -1;
    }
