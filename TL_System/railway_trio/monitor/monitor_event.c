...
int setup_all_process() {
    ...
    for(i=0; i<PROCESS_NUM; i++) {
        ...
        process[i].pid = fork();
        if (process[i].pid  < 0)   
            logerr("%s process error: pid = %d\n", __func__,  process[i].pid);
        else if (process[i].pid == 0) {
            loginfo("%s  start %s success: pid %d\n", __func__, process[i].bin, getpid());

            ret = execv(process[i].bin, process[i].arg);
            if(ret < 0) {
                logerr("process execv error: return %d(%s)", errno, strerror(errno));
                exit(-1);
            }
        }
        sleep(1);
...
int reboot_software() {
    ...
    loginfo("---------------------------------------\n");
    loginfo("software reboot at %s\n", asctime(timenow));
    loginfo("---------------------------------------\n");

    ret = setup_all_process();
    if(ret < 0) {
        logerr("re-setup all process error: return %d\n", ret);
        reboot_hardware();
        return ret;
    }
...
int register_client_module() {
    ...
    loginfo("monitor register to core\n");
