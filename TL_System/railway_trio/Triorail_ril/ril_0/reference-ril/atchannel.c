...
static ATAllStringHandler s_AtStringHandler;
#endif
...
#ifdef FEA_REPORT_ALL_AT_STRING	  
void at_registerAllStringHandler(ATAllStringHandler h)
{
    s_AtStringHandler = h;
}
#endif
                    ...
                    long long timeoutMsec, ATResponse **pp_outResponse)
{
    ...
    while (sp_response->finalResponse == NULL && s_readerClosed == 0) {
        ...
        if (err == ETIMEDOUT) {
            ...
            RLOGD("at_send_command_full_nolock %d\n", err);
            goto error;
        }
        if (err != 0) {
            RLOGD("at_send_command_full_nolock %d\n", err);
...
int at_handshake()
{
    ...
    for (i = 0 ; i < HANDSHAKE_RETRY_COUNT ; i++) {
        ...
        err = at_send_command_full_nolock ("ATE0Q0V1", NO_RESULT,
                    NULL, NULL, HANDSHAKE_TIMEOUT_MSEC*(i+1), NULL);
