...
#define LOG_TAG "REFERENCE_RIL_0"
#include <log.h>
...
static void requestDial(void *data, size_t datalen, RIL_Token t)
{
    ...
    if (ret < 0 || p_response->success == 0) {
        RIL_onRequestComplete(t, RIL_E_GENERIC_FAILURE, p_response->finalResponse, 
            strlen(p_response->finalResponse));
    } else {
        RIL_onRequestComplete(t, RIL_E_SUCCESS, NULL, 0);
    }
...
static void initializeCallback(void *param)
{
    ...
    at_send_command("AT^sm20=0", NULL);
#endif
...
static void onUnsolicited (const char *s, const char *sms_pdu)
{
    ...
    if (strStartsWith(s, "%CTZV:")) {
        ...
        if (strStartsWith(s,"+CRING:") || strStartsWith(s,"RING")) {
            source = RIL_CALL_RING;
        } else if (strStartsWith(s,"NO CARRIER")) {
            source = RIL_CALL_NO_CARRIER;
        } else if (strStartsWith(s,"+CCWA")) {
            source = RIL_CALL_CCWA;
        }
        RIL_onUnsolicitedResponse(RIL_UNSOL_RESPONSE_CALL_STATE_CHANGED,
                                  &source, sizeof(source));
#else    
...
//dir: read=1, write=0
void processAllATString(const char *s, int dir) 
{
    RIL_ATString ATString;
    RLOGI("processAllATString  AT:%s, dir:%d", s, dir);
    ATString.AT= s;
    ATString.dir= dir;
    RIL_onUnsolicitedResponse(RIL_UNSOL_AT_STRING_IND,
                                  &ATString, sizeof(RIL_ATString));
}
    ...
    for (;;) {
        ...
        while  (fd < 0) {
            if (s_port > 0) {
                ...
                fd = tty_init(s_device_path,B19200,0,8,1,'N');
            }
...
#ifdef FEA_REPORT_ALL_AT_STRING	        
        at_registerAllStringHandler(processAllATString);
#endif           
