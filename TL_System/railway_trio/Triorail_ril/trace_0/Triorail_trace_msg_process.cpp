...
static void processL2Message(Trio_trace_frame * pTraceFrame)
{
    ...
    WriteFile_L2(L2Msg);
    PreprocessL2Message(pTraceFrame);    
}
...
static void processL3Message(Trio_trace_frame * pTraceFrame)
{
    ...
    if (iL3MsgLen> 0xFF) {
        RLOGE("processL3Message, error. iL3MsgLen should be 2 byte.");  
    }
    ...
    RLOGI("processL3Message. nLen=%d, iL3MsgLen=%d", nLen, iL3MsgLen);  
    if (iL3MsgLen>(nLen-9)) {
        RLOGE("processL3Message, error. iL3MsgLen err.");  
    }
...
static void resetCellInfo(cellInfo & cell)
{
    cell.nRxLevel = 0x7fff;
    cell.nBCCH = 0;
    cell.nBCC = 0xff;
    cell.nNCC = 0xff;
    cell.nC1 = 0x7fff;
...
void processL1InfoDedicated(Trio_trace_frame * pTraceFrame)
{
    ...
    resetCellInfo(g_L1Msg.servingCellInfo);
...
{
    resetCellInfo(neighbourCellInfo);
