...
static int clients_data_changed() {
    ...
    if (dev_fd > 0 && FD_ISSET(dev_fd, &fd_sets)) {
        ret = recv_device();
        if(ret < 0) {
            logerr( "recv and process gps device frame error %d\n", ret);
            return ret;
        }
    }
    if (core_fd > 0 && FD_ISSET(core_fd, &fd_sets)) {
        ...
        if(ret < 0) {
            logerr( "recv local server error %d; and retry to connect to local server \n", ret);
            return ret;
        }
    }
    return ret;
...
int process_gps_queue() {
    int ret;
    ret = check_and_process_kflag();
    if(ret < 0) {
        logerr("check_and process kflag error: return = %d\n", ret);
        return ret;
    }
...
int64_t get_minimal_interval() {
    return get_kflag_interval();
}
...
int setup_gps_event_loop() {
    ...
    while (event_loop) {
        ...
        ret = set_select_sets();
        if(ret < 0) {
            logerr( "set select sets error: return = %d\n", ret);
            exit(-1);
        }
        ...
        timer.tv_sec = 0;
        timer.tv_usec = get_minimal_interval();
        ret = select(get_sets_max()+1, &fd_sets, NULL, NULL, &timer);
        if (ret < 0) {
            logerr( "select error: return = %d\n", ret);
            exit(-1);
        } else if (ret == 0) {
            logdebug("select timeout: return = %d\n", ret);
        }
