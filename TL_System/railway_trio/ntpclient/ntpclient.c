...
#define DEFAULT_NTP_HOST_1 "ntp.ubuntu.com"
#define DEFAULT_NTP_HOST_2 "time.nist.gov"
...
#ifdef PROJECT_FSL
    tv_set.tv_sec += 8*60*60; // using beijing time zone.
#endif
...
int main(int argc, char *argv[]) {
	...
	ntpc.live=0;
	ntpc.set_clock=0;
	ntpc.probe_count=10;           /* default of 0 means loop forever */
	ntpc.cycle_time=3;          /* seconds */
	ntpc.goodness=0;
	ntpc.cross_check=1;
	...
	if (hostname == NULL) {
		hostname = DEFAULT_NTP_HOST_1;
		printf("use the default NTP host.\n");
	}
