...
static int report_status_info() {
    ...
    module_status =  update_module_status();
    if(running != NULL) {
...
void check_and_report_status_and_position_info() {
    ...
    if(ret >= REPORT_INTERVAL_SEC) {
        logdebug("report status and position info");
        old_moment = current;
        ...
        if(ret < 0) {
            logerr("get trio0 status error");
        }
        ret = get_trio1_status();
        if(ret < 0) {
            logerr("get trio1 status error");
        }
        ret = get_trace_status();
        if(ret < 0) {
            logerr("get trace status error");
        }

        report_position_info();
        loginfo("report position");
        loginfo("report status info");
        report_status_info();
    }
...
int status_task_process(struct frame_fmt *fhp) {
    ...
    int ret;
    struct task_t *tp;
    logdebug("upload status task process");

    switch(fhp->task_state) {


        case REPORT_SUCCESS:
            loginfo("report status success");
            heart_beat = 3;
