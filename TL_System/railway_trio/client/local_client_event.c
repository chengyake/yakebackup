...
int check_usb_prot() {
    ...
    if(access("/dev/ttyUSB2", F_OK) != 0) { //don't exist
        flag++;
    } else {
        flag = 0;
    }
...
int update_module_status() {
    ...
    if(trio0_f != 0) {
        logerr("get trio 0 status error");
        return trio0_f;
    }
    if(trio1_f != 0) {
        logerr("get trio 1 status error");
        return trio1_f;
    }
    if(trace_f != 0) {
        logerr("get tarce status error");
        return trace_f;
    }
    if(gps_f != 0) {
        logerr("get gps status error");
        return gps_f;
    }
    if(gprs_f != 0) {
        logerr("get gprs status error");
        return gprs_f;
    }

    if(check_avail_size() < 100) {// less then 100MBytes
        return -410;
    }
...
int register_client_module() {
    ...
    loginfo("client register to core");
...
#if 0
    struct cmd_header cmd_rsp;
    logdebug("client register to core");
    cmd_rsp.host = IDX_USER_CLIENT;
...
static int register_client_module_rsp() {
    ...
    logdebug("register client to core success");
    return 0;
}

void set_dial_timer(unsigned int sec) {

    if(sec > 300) {
        logerr("dial timeout or access_interval error, timeout is %u", sec);
        return;
    }
    
    if(sec == 0) {
        loginfo("dial timeout cancel");
        dial_deadline = 0;
    } else {
        loginfo("dial timeout or access_interval is %u", sec);
        dial_deadline = time(NULL)+sec;
    }
    
    return;
}

int dial_tele0(struct task_t *t) {
    
    int i;
    unsigned char bf[256];
    unsigned int len = t->phone_len;
    
    if(len > 15) {
        logerr("dial tele0 error");
        return -1;
    }

    set_dial_timer(t->access_interval);
    ...
    memcpy(&bf[CMD_HEADER_SIZE+sizeof(RIL_dial_req)], t->phone_num, len+1);
...
int stop_trace(unsigned char *file, unsigned int len) {
    ...
    if(len > 255) {
        logerr("stop trace error");
        return -1;
    }

    if(tele.state == TELE_ONLINE || dial_deadline > time(NULL)) 
    {
...
int get_client_status_rsp(struct cmd_header *chp) {
    ...
    logdebug("get client statue rsp");
    cmd_rsp.host = IDX_USER_CLIENT;
...
int64_t get_local_hungup_timeout_interval() {
    ...
    if(dial_deadline > t) {
        return dial_deadline - t;
    } else {
        hangup_tele0();
        dial_deadline = 0;
        loginfo("dial timeout and hangup tele0");
    }
...
int64_t get_local_tele_interval() {
    int64_t tmp;
    struct task_t *t;
...
int64_t get_local_interval() {
    ...
    t1 = get_local_hungup_timeout_interval();
    t2 = get_local_tele_interval();
...
int check_and_process_local_task() {
    ...
    if(t->status == TASK_TESTING_AND_UPLOADING) {
        ...
        tmp = time(NULL) - tele.begin;
        if(tmp < 0) {
            logerr("local task time - tele.begin < 0, date error maybe");
            return -1;
        }
        
        if(tele.state == TELE_IDLE) { //first ,we need to start trace
            start_trace(t);
            update_tele_state(TELE_WAIT);
            return 0;
        }

        if(t->online_interval == 0) { //in case of normal after trace on
            if(tele.state == TELE_OFFLINE) {
                    update_tele_state(TELE_WAIT);
                    dial_tele0(t);
            }
...
int process_local_event(struct cmd_header *chp) {
    ...
    switch (chp->cmd) {
        ...
        case GPS_MSG_GET_GPS_INFO_RSP:
            if(chp->len <= 0) {
                logerr("get gps data error: len = 0");
                break;
            }
