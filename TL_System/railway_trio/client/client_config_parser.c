...
#define CONFIG_FILE     "/home/bin/config.txt"

static int compare_string(char *org, char *dst, int len) {
    int i;
    for(i=0; i<len; i++) {
        if(org[i] != dst[i]) {
            return -1;
 ...
 */
static int get_config(char * data, int d_len,  char *key, int k_len, char *target) {
...
int get_train_name(char *uname) {
    ...
    if(fd < 0) {
        printf("config parser: open config file error %d", fd);
        return 0;
    }
    
    len = read(fd, &data[0], MAX_SIZE_OF_FILE);
    if(len <= 0) {
        printf("config parser: read config file error %d", fd);
        return 0;
    }
