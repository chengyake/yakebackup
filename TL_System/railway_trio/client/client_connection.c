...
void debug_print_header(struct frame_fmt *p) {
    ...
    //loginfo("head:0x%2X len:%d token:0x%08X", p->head, p->len, p->token);
    //loginfo("task_type=%d task_state=%4d task_id:0x", p->task_type, p->task_state); 
    for(i=0; i<FRAME_TASK_ID_SIZE; i++) {
    	sprintf(&pbf[i*3], "%02X ", p->task_id[i]);
    }
    loginfo("head:0x%2X len:%d token:0x%08X task_type=%d task_state=%4d task_id:%s", 
            p->head, p->len, p->token,  p->task_type, p->task_state, &pbf[0]);
}
...
void debug_print_data(struct frame_fmt *p) {
    ...
    logdebug("%ld bytes data:0x", p->len-FRAME_FMT_SIZE);
...
static int clients_data_changed() {
    ...
    if (remote_fd > 0 && FD_ISSET(remote_fd, &fd_sets)) {
        ...
        if(ret < 0) {
            logerr("recv remote server error %d; and retry to connect to server ", ret);
            return ret;
        }
    }
    if (local_fd > 0 && FD_ISSET(local_fd, &fd_sets)) {
        ...
        if(ret < 0) {
            logerr("recv and process error %d", ret);
            return ret;
        }
    }
    return ret;
...
int process_task_queue() {
    ...
    if(ret < 0) {
        logerr("check and process remote task error: return = %d", ret);
        return ret;
    }
    ret = check_and_process_local_task();
    if(ret < 0) {
        logerr("check and process local task error: return = %d", ret);
        return ret;
    }
...
void signal_handle(int signo) {
    ...
    if(signo == 3) {
        logerr("signal SIGQUIT");
        exit_client_event_loop();
    }
...
int setup_event_loop() {
    ...
    while (event_loop) {
        ...
        ret = set_select_sets();
        if(ret < 0) {
            logerr("set select sets error: return = %d", ret);
            sleep(1);
        }
        ...
        logdebug("<< timeout:%d >>",(int) timer.tv_sec);

        ret = select(get_sets_max()+1, &fd_sets, NULL, NULL, &timer);//too fast
        if (ret < 0) {
            logerr("select error: return = %d", ret);
            sleep(1);
            continue;
        } else if (ret == 0) {
            logdebug("select timeout: return = %d", ret);
        }

        //logdebug("client data comming");
