...
#define GPS_DEV_TTY     "/dev/ttyUSB0"
...
void *read_loop_thread_func() {
    ...
    while (buf[0] != 0xca && buf[0] != 0xaa) {
        ...
        len = read(dev_fd, &buf[0], 128);
        if(len < 0) {
            syslog(LOG_ERR, "read gps device error1: %d\n", errno);
            return ;
        }
        /*
        for(i=0; i<len; i++) {
...
int read_loop_thread() {
    int ret;
	pthread_t pid;

	ret = pthread_create(&pid, NULL, (void *)read_loop_thread_func, NULL);
	if(ret != 0) {
...
int write_loop_main() {
    ...
    while(1) {
        ...
        write(dev_fd, cmd_line_p, strlen(cmd_line_p));
    }
...
int main() {
    ...
    if(dev_fd < 0) {
        syslog(LOG_ERR, "open GPS_DEV_TTY %s error: %d\n", GPS_DEV_TTY, errno);
        return dev_fd;
    }
