...
int32_t init_kernel_platform() {
    ...
    param=(struct matrix_info_t *)matrix;

    rdp = (char *)malloc(sizeof(int32_t)*YUAN_NUM_OF_MATRIX*YUAN_NUM_OF_MATRIX);
    if(rdp == NULL) {
	...
	inputBuffer = clCreateBuffer(context, CL_MEM_READ_WRITE | CL_MEM_COPY_HOST_PTR, 
            YUAN_NUM_OF_MATRIX*SIZE_OF_YUAN,(void *) (&matrix[YUAN_IDX_OF_INPUT_S]), NULL);
	randomBuffer = clCreateBuffer(context, CL_MEM_WRITE_ONLY | CL_MEM_COPY_HOST_PTR, 
            sizeof(int32_t)*YUAN_NUM_OF_MATRIX*YUAN_NUM_OF_MATRIX, (void *) rdp, NULL);
	parameterBuffer = clCreateBuffer(context, CL_MEM_WRITE_ONLY | CL_MEM_COPY_HOST_PTR, 
            sizeof(struct matrix_info_t),(void *) param, NULL);
...
int32_t set_random_data(uint32_t y, uint32_t d, long value) {
    ...
    if(y<YUAN_IDX_OF_INPUT_S || y >= YUAN_NUM_OF_ALL || d < YUAN_IDX_OF_INPUT_S || d >= YUAN_NUM_OF_ALL) {
        return -1;
    }

    rdp[YUAN_NUM_OF_MATRIX*y + d] = value;
...
static int32_t set_input_to_kernel() {
    ...
    memcpy(p, (char *)(&matrix[YUAN_IDX_OF_INPUT_S]), YUAN_NUM_OF_INPUT*SIZE_OF_YUAN);
...
static int32_t set_random_to_kernel() {
    ...
    char* p = (char*) clEnqueueMapBuffer(commandQueue, randomBuffer, CL_TRUE, CL_MAP_WRITE, 0, 
            YUAN_NUM_OF_MATRIX*YUAN_NUM_OF_MATRIX*sizeof(int32_t), 0, NULL, NULL, &ret);
    if(ret < 0) {
        printf("Error: clEnqueueMapBuffer failed. when set random\n");
        return ret;
    }

    memcpy(p, (char *)rdp, YUAN_NUM_OF_MATRIX*YUAN_NUM_OF_MATRIX*sizeof(int32_t));
...
static int32_t set_parameter_to_kernel() {
    ...
    char* p = (char*) clEnqueueMapBuffer(commandQueue, parameterBuffer, CL_TRUE, CL_MAP_WRITE, 0, 
            sizeof(struct matrix_info_t), 0, NULL, NULL, &ret);
    if(ret < 0) {
        printf("Error: clEnqueueMapBuffer failed. when set parameter：w\n");
        return ret;
    }

    memcpy(p, (char *)param, sizeof(struct matrix_info_t));
...
static int32_t get_matrix_from_kernel() {
    ...
    char* p = (char*) clEnqueueMapBuffer(commandQueue, inputBuffer, CL_TRUE, CL_MAP_READ, 0,
            YUAN_NUM_OF_MATRIX*SIZE_OF_YUAN, 0, NULL, NULL, &ret);
    if(ret < 0) {
        printf("Error: clEnqueueMapBuffer failed. when get matrix\n");
        return ret;
    }
    memcpy((char *)&matrix[YUAN_IDX_OF_INPUT_S], p, YUAN_NUM_OF_MATRIX*SIZE_OF_YUAN);
...
static int32_t run_kernel() {
    ...
    cl_int ret;
    size_t global_work_size[1] = {YUAN_NUM_OF_MATRIX};


    input_algo_callback_by_core();
    set_input_to_kernel();

    if(param->ticks%YUAN_FEEDBACK_TICKS == 0) {
        ret = get_random(rdp, sizeof(int32_t)*YUAN_NUM_OF_MATRIX*YUAN_NUM_OF_MATRIX);
        if (ret < 0) {
...
int32_t suspend_kernel() {
    ...
    param->system_state = SUSPEND_STATE;
    return 0;
...
int32_t resume_kernel() {
    ...
    param->system_state = RESUME_STATE;
    sem_post(&suspend_sem);
    return 0;
}
...
int32_t sync_kernel() {
    ...
    if(param->system_state == RESUME_STATE) {
        return -1;
    }
