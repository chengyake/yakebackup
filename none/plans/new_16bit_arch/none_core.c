...
void init_none() {
    ...
    if(access(FILE_OF_MATRIX, F_OK|R_OK|W_OK) != 0) { //don't exist
        ...
        memset(data.sta, 0x55, NOA*SLOT*sizeof(unsigned short));
        for(i=0; i<NOA; i++) {
            data.idx = i;
...
void sumn_to_table(int first) {
    ...
    for(i=0; i<NOM; i++) { if((i>=NOI&&i<NOO+NOI) || som[i] == NULL) {continue;}
        int j, k;
        for(j=NOI; j<NOA; j++) { if(som[i]->idx==j)continue;
            #ifndef NON_TRAIN
            if(first) {
                unsigned int s=0;
                int r;
...
void output_none() {
    ...
    //printf("%d %d\n", idata[0], sum_p[1]);
    if(idata[0] != 0 && sum_p[1] >= THESHOLD) {
        debug_c++;
        feedback(50);
    } else if(idata[0] != 0 && sum_p[1] < THESHOLD) {
        feedback(-50);
    } else if(idata[0] == 0 && sum_p[1] >= THESHOLD) {
        feedback(-50);
    }
...
void run() {
    ...
    get_random((unsigned char*)&rt[0][0], sizeof(unsigned int)*NOM*NOA);
    get_input_data();
    for(i=0; i<20; i++) {
        input_none();
