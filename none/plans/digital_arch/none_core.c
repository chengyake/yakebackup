...
size_t global_work_size[2] = {32, 64};
...
static cl_mem inputBuffer_idata;
static cl_mem inputBuffer_rdata;
static cl_mem inputBuffer_sum;
static cl_mem inputBuffer_mid;
static cl_mem inputBuffer_all_table;
#endif
...
int32_t init_kernel_platform() {
	...
	if (dev_num == 0) {
		printf("Opencl: No GPU device available.\n");
		printf("Opencl: Choose CPU as default device.\n");
	...
	inputBuffer_idata = clCreateBuffer(context, CL_MEM_READ_ONLY | CL_MEM_COPY_HOST_PTR, 
...
void get_input_data(int ticks) {
    int i;
    if((seed[0]&0x00000001) != 0x0) {
        idata[0] = 10.0;
    } else {
        idata[0] = 0.0;
    }

    if((seed[1]&0x00000001) != 0x0) {
        idata[1] = 10.0;
    } else {
        idata[1] = 0.0;
    }
...
void mid_to_table(int first) {
    ...
    for(i=0; i<NOA; i++) { if(i>=NOI && i<NOO+NOI){continue;}
        ...
        for(j=NOI; j<NOA; j++) { if(i==j) {continue;}
            ...
            if(first) {
                int m=0, id=0;
                for(k=0; k<41; k++) {
                    if(m<all_table[i].sta[j][k]) {
                        m=all_table[i].sta[j][k];
                        id=k;
                    }
                }
                all_table[i].dis[j] = id;
            }
            #endif
            //table to sum
...
void output_none(int ticks) {
    ...
    if(ticks >= 999) {
        printf("active percent %d%%\n",debug_sta*1000/750);
        return;
    }

    char fb;
    char ac= idata[0]==0.0?0:1 + idata[1]==0.0?0:2;
    if(ac == idx) {
        fb = 1;
    } else {
        fb = -1;
    ...
    if(fb != 0) {
        ...
        //printf("fb %d\n", fb);
        #ifndef NON_TRAIN
        for(x=0; x<NOA; x++) { if(x>=NOI && x<NOO+NOI) {continue;}
...
void run() {
    ...
    for(ticks=1; ticks<1000; ticks++) {   //in -1; fb -1;
        get_random((char *)&rdata[0][0], sizeof(rdata));  //71ms
        get_input_data(ticks);
