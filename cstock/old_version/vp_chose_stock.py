...
PRICE_BUY_LINE=0.18  #放量最高价之下N点买入
...
def initialize(context):
    ...
    if debug == 1:
        ...
        enable_profile()
        s = get_index_stocks('000002.XSHG')
        #j=random.randint(1,len(s)-50)
        j=0
        stocks=s[j:j+100]
        
    elif debug == 0:
        stocks=get_index_stocks('000002.XSHG')
...
def volume_price_by_min(context, data, stock):
    ...
    if stage == 1:
        ...
        if  hold > 0 and five_price > 0 and avg > avg_3 and (top_price - gp.price)/top_price > PRICE_BUY_LINE:
